  ...
  $rootScope.getFilterData = function (filter) {
    Utils.loadDogsTerms(filter, $rootScope.petKind).then(function (rep) {
      ...
      for (var key in rep.facets) {
        ...
        var res = string.replace('<select class="facetwp-dropdown">', '<div class="item-select"><div class="input-label">' + $rootScope.filterTitle[newkey] + '</div><select class="filter-select" ng-model="filter.' + newkey + '" ng-change="updateFilters()">');
        rep.facets[key] = res;
      }
    });
  };
          ...
          $scope.loadPosts = function () {
            ...
            DataLoader.all($scope.postsApi + '&' + $rootScope.callback).success(function (data, status, headers, config) {
              ...
              if (data.length)
                  ...
                  $scope.countFilter = "Showing " + $rootScope.totalCount + " Dogs";
                else
                  $scope.countFilter = "No dogs found";
              $ionicLoading.hide();
              //console.dir( data );
            }).
          ...
          $scope.loadMore = function () {
            ...
            $timeout(function () {
              ...
              DataLoader.all(url).success(function (data, status, headers, config) {
                ...
                $scope.moreItems = (data.length >= 10);
                if (data.length)
                  //$scope.countFilter = "Showing " + $scope.posts.length + " of " + $rootScope.totalCount + " Dogs";
                  $scope.countFilter = "Showing " + $rootScope.totalCount + " Dogs";
          ...
          $scope.loadPosts = function () {
            ...
            DataLoader.all($scope.postsApi + '&' + $rootScope.callback).success(function (data, status, headers, config) {
              ...
              if (data.length)
                  ...
                  $scope.countFilter = "Showing " + $rootScope.totalCount + " Cats";
                else
                  $scope.countFilter = "No cats found";
              $ionicLoading.hide();
              //console.dir( data );
            }).
                    error(function (data, status, headers, config) {
          ...
          $scope.loadMore = function () {
            ...
            $timeout(function () {
              ...
              DataLoader.all(url).success(function (data, status, headers, config) {
                ...
                $scope.moreItems = (data.length >= 10);
                if (data.length)
                  //$scope.countFilter = "Showing " + $scope.posts.length + " of " + $rootScope.totalCount + " Cats";
                  $scope.countFilter = "Showing " + $rootScope.totalCount + " Cats";
                // Don't strip post html
                $scope.content = $sce.trustAsHtml(data.content);
                $scope.title = $sce.trustAsHtml(data.title);
          ...
          $scope.loadPosts = function () {
            ...
            DataLoader.all($scope.postsApi + '&' + $rootScope.callback).success(function (data, status, headers, config) {
              ...
              if (data.length)
                  ...
                  $scope.countFilter = "Showing " + $rootScope.totalCount + " Others";
                else
                  $scope.countFilter = "No results found";
              $ionicLoading.hide();
              //console.dir( data );
            }).
                    error(function (data, status, headers, config) {
          ...
          $scope.loadMore = function () {
            ...
            $timeout(function () {
              ...
              DataLoader.all(url).success(function (data, status, headers, config) {
                ...
                $scope.moreItems = (data.length >= 10);
                if (data.length)
                  //$scope.countFilter = "Showing " + $scope.posts.length + " of " + $rootScope.totalCount + " Others";
                  $scope.countFilter = "Showing " + $rootScope.totalCount + " Others";
                // Don't strip post html
                $scope.content = $sce.trustAsHtml(data.content);
                $scope.title = $sce.trustAsHtml(data.title);
